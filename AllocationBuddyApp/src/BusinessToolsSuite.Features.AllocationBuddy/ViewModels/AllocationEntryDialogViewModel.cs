...
public partial class AllocationEntryDialogViewModel : ObservableObject
{
    ...
    /// </summary>
public AllocationEntry ToEntity()    {        var entry = new AllocationEntry        {            ItemNumber = ItemNumber,            Description = Description,            StoreId = StoreId,            StoreName = string.IsNullOrWhiteSpace(StoreName) ? null : StoreName,            Quantity = Quantity,            Rank = Rank,            Category = string.IsNullOrWhiteSpace(Category) ? null : Category,            UnitPrice = UnitPrice > 0 ? UnitPrice : null,            AllocationDate = AllocationDate,            Notes = string.IsNullOrWhiteSpace(Notes) ? null : Notes,            Id = EntryId ?? Guid.NewGuid()        };        return entry;
    }
