                        ...
                        foreach (var f in filesObj)
                        {
                            try
                            {
                                ...
                                if (openRead != null)
                                {
                                    ...
                                    if (stream != null)
                                    {
                                        // try to preserve extension from available name properties
                                        string? originalName = null;
                                        try
                                        {
                                            var nameProp = fType.GetProperty("Name") ?? fType.GetProperty("DisplayName") ?? fType.GetProperty("FileName");
                                            if (nameProp != null)
                                            {
                                                originalName = nameProp.GetValue(f) as string;
                                            }
                                        }
                                        catch { }
                                        ...
                                        {
                                            try { ext = Path.GetExtension(originalName) ?? string.Empty; } catch { ext = string.Empty; }
                                        }

                                        var tmp = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName() + ext);
