...
public partial class InventoryItemDialogViewModel : ObservableObject
{
    ...
    /// </summary>
public InventoryItem ToEntity()    {        var item = new InventoryItem        {            ItemNumber = ItemNumber,            Description = Description,            BinCode = string.IsNullOrWhiteSpace(BinCode) ? null : BinCode,            QuantityOnHand = QuantityOnHand,            MinimumThreshold = MinimumThreshold,            MaximumThreshold = MaximumThreshold,            UnitCost = UnitCost,            UnitPrice = UnitPrice,            Category = string.IsNullOrWhiteSpace(Category) ? null : Category,            LastUpdated = LastUpdated,            Id = ItemId ?? Guid.NewGuid()        };        return item;
    }
