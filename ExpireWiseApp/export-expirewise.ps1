param(
    ...
    [string]$OutDir = "D:\CODE\Cshp\ExpireWiseApp\src"
)
...
$items = @(
    ...
    "BusinessToolsSuite\src\Features\BusinessToolsSuite.Features.ExpireWise",
    "BusinessToolsSuite\src\BusinessToolsSuite.Tools\ImportTester"
)

Write-Host "Exporting ExpireWise related projects from $RepoRoot to $OutDir"
...
$solutionPath = Join-Path $solutionDir "ExpireWiseApp.sln"
if (Get-Command dotnet -ErrorAction SilentlyContinue) {
    Write-Host "Creating solution at $solutionPath"
    dotnet new sln -o $solutionDir -n ExpireWiseApp --force | Out-Null
