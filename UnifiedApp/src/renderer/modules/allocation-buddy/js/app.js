        ...
        complete: function(results) {
          ...
          setTimeout(() => {
            try {
              ...
              showErrorDialog('Error processing data: ' + error.message);
            }
          }, 400);
        },
...
function processData(data) {
  ...
  function detectColumnsUsingDictionary(data, columns) {
    ...
    const storeCol = columnScores.reduce((best, curr) =>
    ...
    // Also check if numeric values look like store IDs (3-digit numbers like 101-199)
    const quantityCol = columnScores.reduce((best, curr) => {
      ...
      // Check if values look like store IDs (typically 101-199 range)
  ...
  setTimeout(() => {
    ...
    const sections = [
      ...
      document.querySelector('.store-selection-section'),
...
async function autoSaveArchive() {
  try {
    if (!window.archiveManager) {
      console.warn('Archive manager not available');
      return;
    }
...
function showFileInfo(filePath, rowCount) {
  ...
  // Show clear button when file is loaded
...
async function exportExcludedData() {
  if (excludedStores.size === 0) {
  ...
  if (electronAPI?.saveFile) {
    try {
      ...
      const filePath = await electronAPI.saveFile({
        defaultPath: `excluded_stores_${timestamp}.csv`,
        filters: [
          { name: 'CSV Files', extensions: ['csv'] },
          { name: 'All Files', extensions: ['*'] }
        ]
      ...
      if (filePath) {
        ...
        await electronAPI.writeFile(filePath, csvData);
        showErrorDialog(`Exported excluded stores data`, 'Export Complete', 'success');
      }
    } catch (error) {
      console.error('Export failed:', error);
      showErrorDialog(`Export failed: ${error.message}`, 'Export Error');
    }
  } else {
...
function updateThemeIcon() {
  if (!themeToggle || !themeIcon || !themeLabel) {
    return; // Theme controls not available in this context
  }
...
async function autoSaveDictionary() {
  try {
    const electronAPI = window.parent?.electronAPI || window.electronAPI;
    if (!electronAPI?.saveDictionary) {
      console.warn('Dictionary save not available');
      return;
    }
    
    const result = await electronAPI.saveDictionary(window.DICT);
    if (result.success) {
...
window.saveDictionaryToFile = async function() {
  try {
    const electronAPI = window.parent?.electronAPI || window.electronAPI;
    if (!electronAPI?.saveDictionary) {
      showErrorDialog('Dictionary save not available in this environment', 'Feature Unavailable');
      return;
    }
...
window.exportDictionary = async function() {
  try {
    const electronAPI = window.parent?.electronAPI || window.electronAPI;
    if (!electronAPI?.exportDictionary) {
      showErrorDialog('Dictionary export not available in this environment', 'Feature Unavailable');
      return;
    }
    
    const result = await electronAPI.exportDictionary(window.DICT);
...
window.importDictionary = async function() {
  try {
    const electronAPI = window.parent?.electronAPI || window.electronAPI;
    if (!electronAPI?.importDictionary) {
      showErrorDialog('Dictionary import not available in this environment', 'Feature Unavailable');
      return;
    }
    
    const result = await electronAPI.importDictionary();
...
// Add event listener to view archives button
if (viewArchivesBtn) {
  viewArchivesBtn.addEventListener('click', openArchiveModal);
}
...
// ============================================
// PostMessage Handler for Unified Settings Integration
// ============================================

/**
 * Listen for messages from parent window (unified settings)
 * Handles commands to open archives, dictionary, and rankings
 */
    ...
    switch (event.data.type) {
      case 'openArchives':
        ...
        if (typeof openArchiveModal === 'function') {
          ...
          console.warn('openArchiveModal function not available');
        }
        break;

      case 'openDictionary':
        // Open the settings modal (dictionary)
        if (typeof openSettingsModal === 'function') {
          openSettingsModal();
          // Switch to the specified tab (items or stores)
          const tab = event.data.tab || 'items';
          if (typeof switchSettingsTab === 'function') {
            switchSettingsTab(tab);
          }
        } else {
          console.warn('openSettingsModal function not available');
        }
        break;

      case 'openRankings':
        // Open the settings modal to the stores tab for managing rankings
        if (typeof openSettingsModal === 'function') {
          openSettingsModal();
          if (typeof switchSettingsTab === 'function') {
